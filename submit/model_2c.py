...
wv =Word2Vec.load(r'C:\Tianzheng\OneDrive - University of Waterloo\24 S\MSCI 641\project\task-1\archive\cus_w2v.model')

with open(r'C:\Tianzheng\OneDrive - University of Waterloo\24 S\MSCI 641\project\task-1\archive\data\w2vec_bi.pkl', 'rb') as f:
    model = pickle.load(f)
...
def load_to_loader(file_path, test=False):
    ...
    df['feature'] = df.apply(lambda x: np.concatenate([x['postTextVec'], x['targetParagraphsVec'], np.array([x['targetLengths']]), np.array([x['textLengths']])]), axis=1)
