...
class CNNModel(nn.Module):
    def __init__(self, activation_fn='ReLU', input_channels=1, num_classes=3, dropout_rate=0.5):
        ...
        self.conv4 = nn.Conv1d(in_channels=300, out_channels=300, kernel_size=3, padding=1)

        self.fc1InputSize=300*(1413//2//2)
        # Fully connected layers
        self.fc1 = nn.Linear(self.fc1InputSize, 512)
    ...
    def forward(self, x):
        ...
        x = self.activation(self.conv3(x))  # (batch_size, 300, 46)
        x = self.activation(self.conv4(x)) # (batch_size, 300, 46)
        x = self.pool(x)  # (batch_size, 300, 23)
...
def sentence2vec(sentence):
    ...
    sentence_vector = np.mean([wv[word.replace(",","").replace("-","_")] for word in words if word.replace(",","").replace("-","_") in wv] or [np.zeros(w2v_size)], axis=0)
...
def para2vec(x1,x2):
    ...
    sentence_vector = np.array([wv[word.replace(",","").replace("-","_")] if word.replace(",","").replace("-","_") in wv else np.zeros(wv.vector_size) for word in words])
    # sentence_vector size: 93*wv.vector_size
    # padding to this size (for sentences with less than 93 words)
    sentence_vector = sentence_vector[:word_1_threshold]
    sentence_vector = np.pad(sentence_vector, ((0, word_1_threshold - len(sentence_vector)), (0, 0)), 'constant')
    # clipping to this size (for sentences with more than 93 words)

    word_2_threshold=1396
    words=targetParagraphs.split()
    sentence_vector_2 = np.array([wv[word.replace(",", "").replace("-", "_")] if word.replace(",", "").replace("-",
                                                                                                             "_") in wv else np.zeros(
        wv.vector_size) for word in words])

    sentence_vector_2 = sentence_vector_2[:word_2_threshold]
    sentence_vector_2 = np.pad(sentence_vector_2, ((0, word_2_threshold - len(sentence_vector_2)), (0, 0)), 'constant')

    sentence_vector = np.concatenate((sentence_vector, sentence_vector_2), axis=0)
...
def load_to_loader(file_path, test=False):
    ...
    # df["fullpara"] = df["targetParagraphs"].apply(lambda x: " ".join(x))

    df['feature'] = df.apply(lambda x: para2vec(x.postText, x.targetParagraphs), axis=1)
