# input is sentence vector for postText and Paragraphs
...
def para2vec(x1,x2):
    ...
    sentence_vector = np.array([sentence2vec(postText)])
    for targetSen in targetParagraphs:
        sentence_vector = np.concatenate((sentence_vector, [sentence2vec(targetSen)]))

    # padding to 1+41=42*300
    # cutting to 42*300
    sentence_vector = sentence_vector[:42]
    sentence_vector = np.pad(sentence_vector, ((0, 42 - len(sentence_vector)), (0, 0)), 'constant')
